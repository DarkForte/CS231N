...
def affine_bnorm_relu_forward(x, w, b, gamma, beta, bn_param):
  ...
  b, bnorm_cache = batchnorm_forward(a, gamma, beta, bn_param)
  c, relu_cache = relu_forward(b)
  return c, (fc_cache, bnorm_cache, relu_cache)

def affine_bnorm_relu_backward(dout, cache):
  fc_cache, bnorm_cache, relu_cache = cache
  dc = relu_backward(dout, relu_cache)
  db, dgamma, dbeta = batchnorm_backward(dc, bnorm_cache)
...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    scores, softmax_cache = affine_forward(relu_out, self.params["W2"], self.params["b2"])
    ...
    d_aff2, grads["W2"], grads["b2"] = affine_backward(d_softmax, softmax_cache)
    grads["W2"] += self.reg * self.params["W2"]

    d_relu = relu_backward(d_aff2, relu_cache)
    d_aff1, grads["W1"], grads["b1"] = affine_backward(d_relu, hidden_cache)
...
class FullyConnectedNet(object):
  ...
  def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
    ...
    self.params["W%d" % self.num_layers] = np.random.normal(0, weight_scale, size = (hidden_dims[-1], num_classes))

    for i in range(2, self.num_layers):
      self.params["W%d" % i] = np.random.normal(0, weight_scale, size=(hidden_dims[i-2], hidden_dims[i-1]))
  ...
  def loss(self, X, y=None):
    ...
    for i in range(1, self.num_layers):
      ...
      if self.use_batchnorm:
        ...
        hidden_out, hidden_cache = affine_bnorm_relu_forward(hidden_out, w, b, gamma, beta, self.bn_params[i-1])
      else:
        hidden_out, hidden_cache = affine_relu_forward(hidden_out, w, b)
    ...
    hidden_out, hidden_cache = affine_forward(hidden_out, self.params["W%d" % self.num_layers], self.params["b%d" % self.num_layers])
    hidden_outs.append(hidden_out)
    hidden_caches.append(hidden_cache)
    ...
    for i in range(1, self.num_layers+1):
      sum_reg += np.sum(np.square(self.params["W%d" % i]))
    loss += 0.5 * self.reg * sum_reg

    dx, dw, db = affine_backward(d_softmax, hidden_caches[-1])
    ...
    for i in range(self.num_layers-1, 0, -1):
      ...
      if(self.use_batchnorm):
        dx, dw, db, dgamma, dbeta = affine_bnorm_relu_backward(dx, hidden_caches[i-1])
        grads["W%d" % i] = dw + self.reg * self.params["W%d" % i]
        grads["b%d" % i] = db
        grads["gamma%d" % i] = dgamma
        grads["beta%d" % i] = dbeta
      else:
        dx, dw, db = affine_relu_backward(dx, hidden_caches[i-1])
        grads["W%d" % i] = dw + self.reg * self.params["W%d" % i]
        grads["b%d" % i] = db
